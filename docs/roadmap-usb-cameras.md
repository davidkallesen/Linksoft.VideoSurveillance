...
- ✅ Verified `JsonCameraStorageService` against the "delete + re-add same physical device" flow. No production change needed: storage is Guid-keyed (`AddOrUpdateCamera` indexes on `camera.Id`, `DeleteCamera` removes by Guid), and no DeviceId-uniqueness gate exists in the storage path, the camera-create dialog, or the API handlers. The "block" the bullet anticipated does not exist — Guid-keying survived Phase 5 polish unchanged. **Adjacent finding (not fixed):** `UsbCameraLifecycleCoordinator.unpluggedCameras` retains an entry for any camera that was marked unplugged and then deleted from storage — a bounded leak (≤ count of cameras deleted while unplugged), benign because no consumer queries those stale Guids, but a real defect in the coordinator (not the storage). Worth a one-line fix in `OnDeviceArrived` / `DeleteCamera` if Phase ≥10 revisits the coordinator; flagged here so it isn't lost.
