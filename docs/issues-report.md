...
- **G2.2 HLS stream-copy by default** — `StreamingService` FFmpeg args switched from `-c:v libx264 -preset ultrafast -tune zerolatency -g 30` to `-c:v copy`. Modern IP cameras already produce HLS-compatible H.264/H.265, so transcoding was pure CPU waste — at 10+ viewed cameras libx264 saturated the host. Audio still re-encodes (`-c:a aac`) because cameras commonly emit G.711/PCMU which HLS cannot demux.
- **G2.3 Background remuxer write queue** — `Remuxer.WritePacket` previously called `av_interleaved_write_frame` synchronously on the demux thread; a slow drive (cleanup pass on the same disk, network share hiccup) backed up the RTSP buffer and dropped frames upstream. The synchronous write is now an enqueue into a 1000-packet bounded `BlockingCollection` consumed by a dedicated background `Thread`. The writer drains under `syncLock`; `CloseLocked` drains synchronously before close so no packets land in a stale `outputCtx`. Drop-oldest semantics preserve the freshest history if the disk is permanently behind.
- **Server segmentation timer** — `ServerRecordingSegmentationBackgroundService` (`IHostedService` on a `System.Threading.Timer`) drives the new `ServerRecordingService.SegmentRecording` on a 30 s check interval using the shared `RecordingSlotCalculator`. Without this the server would never roll segments even though the implementation exists.
